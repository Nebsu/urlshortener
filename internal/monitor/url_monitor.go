...
func NewUrlMonitor(linkRepo repository.LinkRepository, interval time.Duration) *UrlMonitor {
	return &UrlMonitor{
		linkRepo:    linkRepo,            // Référence au repository de liens
		interval:    interval,            // Intervalle de vérification
		knownStates: make(map[uint]bool), // Initialise la map pour stock
		mu:          sync.Mutex{},        // Initialise le mutex pour la protection concurrentielle
	}
}
...
func (m *UrlMonitor) checkUrls() {
	...
	for _, link := range links {
		...
		if previousState != currentState {
			...
			log.Printf("[MONITOR] L'état du lien %s (%s) reste inchangé : %s",
				link.ShortCode, link.LongURL, formatState(currentState))
		}
	}
...
func (m *UrlMonitor) isUrlAccessible(url string) bool {
	...
	defer resp.Body.Close() // Ferme le corps de la réponse pour éviter les fuites de mémoire
